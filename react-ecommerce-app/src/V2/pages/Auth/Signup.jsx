...
const SignupForm = () => {
    ...
    const valid = (userData) => {
        return !areEmpty(userData) && userData.password === confirmPassword && policyChecked && termsChecked;
    }
            ...
            axios.post('http://localhost:5000/api/auth/signup', userData)
                .then(response => {
                    if (response.status === 201) {
                        main_app_context.navigate('/login');
                    }
        ...
        <form onSubmit={registerUser} role="form">
            ...
            <div className="my-4">
                <div className="custom-control custom-checkbox mb-3">
                    <input onChange={() => setTermsCheck(!termsChecked)} type="checkbox"
                           className="custom-control-input" id="check-terms"/>
                    <label className="custom-control-label" htmlFor="check-terms">I agree to the <a href="#">terms
                        and conditions</a></label>
                </div>
                <div className="custom-control custom-checkbox">
                    <input onChange={() => setPolicyChecked(!policyChecked)} type="checkbox"
                           className="custom-control-input" id="check-privacy"/>
...
const Signup = () => {
    return (
        <Auth title={'Signup'} subTitle={'Create your free account today'} footerText={'Already have an account?'}
              footerLink={'/login'} footerActionText={'Sign in'}>
            <SignupForm/>
        </Auth>
    )
}
